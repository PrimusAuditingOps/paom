...
class PaaSchemesRanking(models.Model):
    ...
    name = fields.Char(string='Ranking', compute='_get_name')

    def _get_name(self):
        for rec in self:
            rec.name = rec.schem_id.name + ' - '+str(rec.rating)

    partner_id = fields.Many2one('res.partner', string='Auditor',
                                 ondelete='set null')
    schem_id = fields.Many2one('paa.assignment.auditor.scheme',
                               string='Scheme',
                               ondelete='set null')
