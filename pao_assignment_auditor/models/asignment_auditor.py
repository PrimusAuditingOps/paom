...
class AssignmentAuditor(models.Model):
    _name = 'assignment.auditor'
    _description = 'Assignment Auditor Functions'

    def get_auditor_assignment(self, dates=None, startdates=None, products=None,
                               organizations= None, saleorderid = None, orderid=None,
                               cityid=None, failed=False, **kwargs):
        ...
        if weightings:
            ...
            if auditors_list:
                ...
                for auditor in auditors_list:
                    ...
                    value = self.env['paoassignmentauditor.auditor.qualification'].sudo().create(
                        {
                            'auditor_id': auditor,
    ...
    def _get_days_without_audits(self, auditor_id):
        ...
        for last_service_date in result:
            ...
            self.env.cr.execute(sql, params)
            result_date = self.env.cr.dictfetchall()
    ...
    def _get_weighting(self):
        recweighting = self.env['paoassignmentauditor.weighting'].search([], limit=1)
    ...
    def _get_schemes(self,products_ids):
        schemes_list = []
        rec_products = self.env['product.product'].browse(products_ids)
    ...
    def _get_auditor_scheme_rating(self,auditor_list, scheme_list, weighting):
        auditor_rating = []
        auditors = self.env['res.partner'].browse(auditor_list)
    ...
    def _get_auditor_localization(self,auditor_list, audit_city_id, weighting, first_start_date):
        ...
        localization_list = []
        auditor_ids = self.env['res.partner'].browse(auditor_list)

        if audit_city_id:
            reccity = self.env['res.city'].browse(audit_city_id)
    ...
    def _get_localization_auditor_audit(self, auditor,first_start_date):
        ...
        if orders_ids_list:
            rec_purchase_order = self.env["purchase.order"].browse(orders_ids_list)
    ...
    def _get_configuration_audit_quantity(self):
        recauditconf = self.env['paoassignmentauditor.configuration.audit.quantity'].search([], limit=1)
    ...
    def _get_audits_quantity_per_auditor(self,auditors_list, weighting, date_list, season_start_month, order_id):
        ...
        for auditor in auditors_list:
            total_audit = 0
            recpartner = self.env['res.partner'].browse(auditor)
    ...
    def _get_calification_audit_quantity(self, products_ids, order_id, auditor, total_audit, start_date, end_date, weighting):
        ...
        if products_ids:                    
            ...
            rec_audit_quantity =  self.env['purchase.order.line'].read_group(domain,['partner_id', 'product_qty'], ['partner_id'])
    ...
    def _get_configuration_audit_honorarium(self):
        recweighting = self.env['paoassignmentauditor.configuration.audit.honorarium'].search([], limit=1)
        for rec in recweighting:
            return rec
        return {}
    ...
    def _get_calification_audit_honorarium(self, products_ids, order_id, auditor, audits_total_honorarium, start_date, end_date, weighting, partner_currency_rate):
        ...
        if products_ids:
            ...
            recpurchaseordersline = self.env['purchase.order.line'].search(domain)
