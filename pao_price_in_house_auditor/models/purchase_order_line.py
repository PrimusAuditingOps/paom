    ...
    def _onchange_product_id_in_house_auditor(self):
        for rec in self:
            ...
            if not rec._origin.product_id:
                ...
                seller = rec._origin.product_id._select_seller(
                    ...
                    params=params)
                if seller:
                    price_unit = rec.env['account.tax']._fix_tax_included_price_company(seller.pao_price_in_house_auditor, rec._origin.product_id.supplier_taxes_id, rec.taxes_id, rec.company_id) if seller else 0.0
