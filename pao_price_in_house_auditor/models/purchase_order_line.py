...
class PurchaseOrderLine(models.Model):
    ...
    def _onchange_product_id_in_house_auditor(self):
        for rec in self:
            ...
            if not rec.product_id._origin:
                rec.pao_price_in_house_auditor = 0.00
                return
            else:
                params = rec._get_select_sellers_params()
                seller = rec.product_id._origin._select_seller(
                    ...
                    params=params)
                if seller:
                    price_unit = rec.env['account.tax']._fix_tax_included_price_company(seller.pao_price_in_house_auditor, rec.product_id._origin.supplier_taxes_id, rec.taxes_id, rec.company_id) if seller else 0.0
