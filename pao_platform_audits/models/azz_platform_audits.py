...
class PaoAzzPlatformAudits(models.Model):
    ...
    not_relate_sol = fields.Many2many(
        'sale.order.line', 
        'pao_azz_platform_audits_not_relate_sol_rel',
        'platform_id', 
        'sale_order_line_id', 
        string='Not relate Sales Order Line',
    )
    ...
    def _compute_purchase_order_line(self):
        for rec in self:
            ...
            if not pol_id and rec.audit_date:       
                ...
                domain = [("service_start_date",">=",date_search),("state","!=","cancel")]
    ...
    def _compute_sale_order_line(self):
        for rec in self:
            if rec.audit_date:
                ...
                domain = [("service_start_date",">=",date_search),("state","!=","cancel"),("company_id","=",rec.company_id.id)]
                sol_id = None
                first_sol_id = None
                if rec.organization_id:
                    domain.append(("organization_id","=",rec.organization_id.id))
                    ...
                    if rec.not_relate_sol:
                        domain.append(("id","not in",rec.not_relate_sol))
    ...
    def unlink_audit(self):
        for rec in self:
            ...
            if rec.sale_order_line_id:
                if rec.search_state == "needs_validation":
                    rec.not_relate_sol = [(4, rec.sale_order_line_id.id)] 
