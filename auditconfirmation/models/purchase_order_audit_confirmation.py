...
class PurchaseOrderAuditConfirmation(models.Model):
    ...
    ac_access_token = fields.Char('Security Token', 
        default=lambda self: str(uuid.uuid4().hex),
        help="Access token to set the audit confirmation of the value",
        readonly=True, copy=False)
    ac_consumed = fields.Boolean(string = "Confirmation consumed",
                                 default = False)
    ac_consumed_travel_expenses = fields.Boolean(default = False,
    ...
    ac_id_purchase = fields.Integer(string='Purchase ID', required=True,
