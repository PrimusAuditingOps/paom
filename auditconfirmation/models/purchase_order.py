...
class PurchaseOrder(models.Model):
    ...
    def _compute_audit_confirmation_status(self):
        for rec in self:
    ...
    ac_audit_confirmation_status = fields.Selection(
        ...
        compute='_compute_audit_confirmation_status',
    ...
    ac_is_an_auditor = fields.Boolean(string = "Is an auditor.",
                                      compute=_get_is_an_auditor)
    ac_request_travel_expenses = fields.Boolean(default=True,
    ...
    def show_audit_state_history(self,context=None):
        ...
        view_id_tree = self.env['ir.ui.view'].sudo().search(
            [('name','=',"purchase.audit.state.history.view.tree")])
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'auditconfirmation.auditstate.history',
            'view_type': 'form',
