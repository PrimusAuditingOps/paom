...
class AuditorState(models.Model):
    ...
    name = fields.Char(required=True, string= "Status")
    show_in_portal = fields.Boolean(string="Show in portal", default= False)
    color = fields.Integer(string="Color", required=True, default = 0)
    default_status = fields.Boolean(string="Default status", default= False)
    ...
    def _validate_default_status(self):
        for rec in self:
            if rec.default_status == True:
                domain = [("id", "<>", rec.id),
                          ("default_status","=", True)] if rec.id else [("default_status","=", True)]
