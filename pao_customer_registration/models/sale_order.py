...
class SaleOrder(models.Model):
    ...
    def create(self, vals):
        for rec in self:
            ...
            if partner: 
                recPartner = rec.env["res.partner"].browse(partner)
                for rec in recPartner:
                    recRegistration = rec.env["pao.customer.registration"].search([("res_partner_id", "=", rec.id)])
                    if not recRegistration:
                        cr = rec.env['pao.customer.registration'].create({
                            'contact_id': rec.id,
            ...
            records = super(SaleOrder, rec).create(vals)
