...
class AccountMove(models.Model):
    ...
    ad_usd_neto = fields.Monetary(compute='_get_usd_neto', string='Neto USD', readonly=True)
    ad_usd_iva = fields.Monetary(compute='_get_usd_iva', string='IVA USD', readonly=True)
    ad_usd_total = fields.Monetary(compute='_get_usd_total', string='Total USD', readonly=True)
    ad_mxn_neto = fields.Monetary(compute='_get_mxn_neto', string='Neto MXN', readonly=True)
    ad_mxn_iva = fields.Monetary(compute='_get_mxn_iva', string='IVA MXN', readonly=True)
    ad_mxn_total = fields.Monetary(compute='_get_mxn_total', string='Total MXN', readonly=True)
    ...
    def _get_rates_currency(self, date):
        ...
        query = """SELECT c.name,
                          COALESCE((SELECT r.rate FROM res_currency_rate r
                                  WHERE r.currency_id = c.id AND r.name <= %s AND r.company_id = %s
                               ORDER BY r.name DESC 
                                  LIMIT 1), 1.0) AS rate
                   FROM res_currency c
                   WHERE c.name = %s"""
        self._cr.execute(query, (date,self.company_id.id,currencyname))
