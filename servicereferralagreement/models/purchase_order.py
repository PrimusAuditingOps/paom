...
class PurchaseOrder(models.Model):
    ...
    audit_fee_id = fields.Many2one('servicereferralagreement.auditfees',
    ...
    audit_city_id = fields.Many2one('res.city', string='Audit City',
                                    help='Select City', ondelete='restrict',
                                    domain=[('state_id', '=', -1)])
    sra_audit_signature = fields.Binary(string="Audit Signature", copy=False)
    sra_audit_signature_name = fields.Char(string="Auditor's signature name",
                                           copy=False)
    sra_audit_signature_date = fields.Date(string="Auditor's signature date",
                                           copy=False)
