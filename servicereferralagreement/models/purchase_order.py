...
class PurchaseOrder(models.Model):
    ...
    def onchange_sale_order_id(self):
        ...
        if not self.sale_order_id:
            ...
            purchase_line = [(5, 0, 0)]
            domain = [('id','!=',self._origin.id),('sale_order_id','=',self.sale_order_id.id),('sale_order_id','=',self.sale_order_id.id),('state','!=','cancel')]
                ...
                if line.product_template_id.can_be_commissionable:
                    ...
                    if add:
                        ...
                        if self.date_planned:
                            dateplanned = self.date_planned
                        else:
                            if self.date_order:
                                dateplanned = self.date_order
                        
                        if self.partner_id:
                            percentagevendor = self.partner_id.vendor_service_percentage
        ...
        if  hasregistration and purchase_line:
            ...
            pedido = self.sale_order_id.name
            self.sale_order_id = None
            return {
                    'warning': {
                        'title': "Warning",
                        'message': _('Sales order {0} does not have products available to link to the purchase order.'.format(pedido)),
                    },
                }
